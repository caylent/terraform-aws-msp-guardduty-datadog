...
The only way to give each region its own provider alias is N individually-written (but `count`-toggleable) module blocks — one hardcoded per real AWS region. That's what `providers.tf` (provider aliases) and `regions.tf` (module calls) at the repo root do; `locals.tf` maps each region string to the underscore-safe alias/block-label used by both, since HCL identifiers can't contain hyphens. `outputs.tf` re-exposes each submodule's outputs as a `merge()` over all ~32 possible region instances (`module` cannot be indexed dynamically by a computed key, so this can't be collapsed into a `for` expression).
